...
## [7.0.4] — 2026-06-04 (Reference-integrity validator + validator self-test)

Ports two self-contained integrity improvements from the live system. The validator suite goes 8 → 9 and gains a meta-test.

- **`references` validator** (`scripts/validate-references.js`, the 9th) — confirms every script/hook that automation points at actually exists on disk: hook commands in `settings.json`, `cleanup-config.json` rule targets, scheduled-task `SKILL.md` invocations, `session-start/stop.sh`, and the `CLAUDE.md` / `REFERENCE.md` manuals. Catches the silent-breakage class where a renamed or deleted script leaves a dangling reference. Conservative extraction (only `hooks/|scripts/|commands/` paths ending `.js/.py/.sh`, with guards against matching `.js` inside `.json` and against mid-word matches) so prose and config filenames never false-positive. Path matching is user-agnostic (works for any `~/.claude` install).
- **Validator self-test** (`scripts/test-validators.js`) — "who validates the validators?" Unit-tests the reference-extraction logic, including the adversarial *must-reject* cases that would fail on a buggy regex, then smoke-tests all 9 validators for crashes and output-shape regressions. Wired into `weekly-validator-sweep` (step 2) and runnable on demand.
- **Shared manifest** (`scripts/lib/validators.js`) — single source of truth for the validator list, consumed by both `system-doctor.js` (refactored off its inline array) and `test-validators.js`, so the two can never drift apart.
...
- M5: `ARCHITECTURE.md` §MCP Servers updated — Obsidian removed from bundled MCP_DOCKER list with explicit degraded note (14 failures 2026-04-09 → 2026-04-10) pointing users to filesystem tools against `<your-wiki-path>`.
...
- M10: Refreshed `<your-wiki-path>/wiki/source/atlas-system.md` to v6.9.1 snapshot — includes CRG integration, auto-history-check, new MCP table, v6.9.1 hook profiles, skill registry entry for graphify/handoff/audit. Was 10 days stale (2026-04-10)
