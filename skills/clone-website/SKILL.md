...
license: MIT
metadata:
  vendored_from: github:JCodesMore/ai-website-cloner-template@58e00d5
  sk_id: SK-145
---

> **In the ATLAS system:** browser automation (Pre-Flight #1) is satisfied by the in-app **Claude_Browser** pane (`mcp__Claude_Browser__*`) — prefer it; `claude-in-chrome` and the `playwright-cli` skill are fallbacks. This skill produces a **Next.js 16 + shadcn/ui + Tailwind v4** codebase and expects that scaffold to exist (Pre-Flight #3); the upstream template pins `engines.node >=24` but the runtime works on the installed Node 22 — if a fresh scaffold is needed, `npx create-next-app@latest` + the `tailwind-setup`/`impeccable` skills set it up. `references/INSPECTION_GUIDE.md` is the extraction checklist. **Clone only sites you have the right to; never for impersonation or passing off a design as your own.**
