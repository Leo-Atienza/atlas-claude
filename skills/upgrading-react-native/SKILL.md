...
metadata:
  ...
  vendored_from: github:callstackincubator/agent-skills@0ba043ab
---
...
> **In the ATLAS app-dev system:** use this for **bare/ejected** React Native version bumps where native iOS/Android config (CocoaPods, Gradle, AppDelegate) must change. For a managed Expo project, prefer `upgrading-expo` — reach for this only once the project has ejected or needs native-side migration Expo's flow doesn't cover. (the user's default stack is Expo/EAS, so this skill triggers on bare-RN/native work, not the managed path.)
