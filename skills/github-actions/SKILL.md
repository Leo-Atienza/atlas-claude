...
> **In the ATLAS app-dev system:** use this for **React Native CI on GitHub Actions** specifically (simulator/emulator builds + artifact download). It complements `expo-cicd-workflows` (EAS Workflows, the managed-Expo path) and `fastlane` (SK-136, native store deploy) — reach for this when the project builds on GitHub Actions rather than EAS. Pairs with `ship-verify` (SK-128) to confirm the produced artifact is real.
