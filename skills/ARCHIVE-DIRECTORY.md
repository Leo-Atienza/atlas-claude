# Archive Skills Directory

> Archived skills organized by domain bundle. Load from the relevant page when needed.
> If a task requires an archived skill, read the page → find the SKILL.md path → load it.

| ID | Name | Purpose | Bundle | Page |
|----|------|---------|--------|------|
| SK-001 | Word Document Tools | Create/read/edit Word documents | Dormant — Document & Media Tools | 7 |
| SK-002 | PDF Tools | Read/merge/split/create PDFs; OCR; form filling | Dormant — Document & Media Tools | 7 |
| SK-003 | Presentation Tools | Create/edit slide decks and presentations | Dormant — Document & Media Tools | 7 |
| SK-004 | Spreadsheet Tools | Open/edit/create spreadsheets; CSV conversion | Dormant — Document & Media Tools | 7 |
| SK-008 | Remotion Video | Programmatic video creation with React | Dormant — Document & Media Tools | 7 |
| SK-052 | LinkedIn Publisher | Compose and publish LinkedIn posts via API | Dormant — Document & Media Tools | 7 |
| SK-057 | L100 Web Orchestrator | Superseded by SK-083 Vanguard Web Architecture | Web (archived) | N/A |
| DV-001–DV-031 | CC-DevOps Suite (31 skills) | Terraform, Ansible, K8s, Helm, Docker, CI/CD generators | Infra & DevOps | 1 |
| FS-029 | Cloud Architect | Multi-cloud architecture patterns | Infra & DevOps | 1 |
| FS-030 | K8s Specialist | Kubernetes orchestration and scaling | Infra & DevOps | 1 |
| FS-031 | DevOps Engineer | CI/CD pipelines, infrastructure automation | Infra & DevOps | 1 |
| FS-047 | Chaos Engineer | Chaos testing and resilience engineering | Infra & DevOps | 1 |
| FS-048 | Microservices Architect | Distributed system design patterns | Infra & DevOps | 1 |
| FS-049 | SRE Engineer | Site reliability, observability, incident response | Infra & DevOps | 1 |
| FS-051 | Monitoring Expert | Prometheus, Grafana, alerting strategies | Infra & DevOps | 1 |
| FS-053 | Terraform Engineer | IaC with Terraform, state management | Infra & DevOps | 1 |
| SC-021 | DevContainer Setup | Development container configuration | Infra & DevOps | 1 |
| FS-032 | Security Reviewer | Security-focused code review (superseded by SC-001/002/003) | Security | 2 |
| SC-004 | Variant Analysis | Code variant analysis for vulnerability patterns | Security | 2 |
| SC-005 | Static Analysis | SAST tooling and configuration | Security | 2 |
| SC-006 | Property-Based Testing | Hypothesis/property-based test generation | Security | 2 |
| SC-007 | Audit Context Building | Security audit preparation and context | Security | 2 |
| SC-008 | Semgrep Rules | Custom Semgrep rule creation | Security | 2 |
| SC-009 | Semgrep Advanced | Advanced Semgrep patterns and CI integration | Security | 2 |
| SC-010 | Spec-to-Code Verification | Verify implementation matches specification | Security | 2 |
| SC-011 | Constant-Time Code | Timing-safe implementations | Security | 2 |
| SC-012 | Testing Handbook | Security testing methodology | Security | 2 |
| SC-013 | Smart Contract Auditing | Solidity/EVM security analysis | Security | 2 |
| SC-014 | Smart Contract Advanced | Advanced blockchain security patterns | Security | 2 |
| SC-015 | Firebase APK Analysis | Android APK security review | Security | 2 |
| SC-016 | YARA Rules | Malware detection rule creation | Security | 2 |
| SC-017 | DWARF Analysis | Binary debugging information analysis | Security | 2 |
| SC-018 | BurpSuite | Web application security testing | Security | 2 |
| SC-019 | Modern Python Security | Python-specific security patterns | Security | 2 |
| SC-022 | GitHub CLI Security | gh CLI security workflows | Security | 2 |
| SC-023 | Git Cleanup | Repository cleanup and history management | Security | 2 |
| SC-024 | Second Opinion | Cross-validation of security findings | Security | 2 |
| SC-025 | Workflow Skill Design | Security workflow design patterns | Security | 2 |
| SC-026 | Chrome Troubleshooting | Browser debugging and security | Security | 2 |
| SC-027 | Culture Index | Security culture assessment | Security | 2 |
| SC-028 | Debug Buttercup | Advanced debugging techniques | Security | 2 |
| FS-004 | Go Expert | Go patterns, concurrency, stdlib | Enterprise | 3 |
| FS-005 | Rust Engineer | Rust ownership, async, systems programming | Enterprise | 3 |
| FS-006 | Swift Expert | Swift patterns beyond iOS (server-side, CLI) | Enterprise | 3 |
| FS-007 | C++ Expert | Modern C++, templates, memory management | Enterprise | 3 |
| FS-008 | C# Expert | .NET patterns, LINQ, async/await | Enterprise | 3 |
| FS-009 | Java Expert | JVM optimization, Spring patterns | Enterprise | 3 |
| FS-010 | Kotlin Expert | Kotlin idioms, coroutines, multiplatform | Enterprise | 3 |
| FS-011 | PHP Expert | Modern PHP, Laravel patterns | Enterprise | 3 |
| FS-013 | FastAPI Developer | Python async API with FastAPI | Enterprise | 3 |
| FS-014 | Django Developer | Django ORM, views, admin, DRF | Enterprise | 3 |
| FS-015 | Rails Developer | Ruby on Rails conventions and patterns | Enterprise | 3 |
| FS-016 | NestJS Developer | NestJS modules, DI, TypeORM | Enterprise | 3 |
| FS-017 | Spring Boot Developer | Spring Boot, JPA, security | Enterprise | 3 |
| FS-018 | Laravel Developer | Laravel Eloquent, middleware, queues | Enterprise | 3 |
| FS-019 | .NET Core Developer | ASP.NET Core, Entity Framework | Enterprise | 3 |
| FS-022 | Vue Expert | Vue 3 Composition API, Pinia, Nuxt | Enterprise | 3 |
| FS-023 | Angular Expert | Angular modules, RxJS, signals | Enterprise | 3 |
| FS-062 | WordPress Expert | WordPress themes, plugins, Gutenberg | Enterprise | 3 |
| FS-063 | Salesforce Expert | Apex, Lightning, Salesforce DX | Enterprise | 3 |
| FS-064 | Legacy Modernizer | Legacy codebase modernization strategies | Enterprise | 3 |
| FS-066 | Secure Code Guardian | Security-focused code generation | Enterprise | 3 |
| FS-027 | Database Optimizer | Query optimization, indexing strategies | Data & ML | 4 |
| FS-040 | ML Pipeline Engineer | ML training, deployment, MLOps | Data & ML | 4 |
| FS-041 | Data Engineer (Spark) | Apache Spark, data pipelines, ETL | Data & ML | 4 |
| FS-056 | Fine-Tuning Expert | LLM fine-tuning, LoRA, training data | Data & ML | 4 |
| FS-057 | RAG Architect | Retrieval-augmented generation systems | Data & ML | 4 |
| FS-058 | Prompt Engineer | Prompt design, evaluation, optimization | Data & ML | 4 |
| SK-016–SK-026 | Mobile Suite (11 skills) | React Native, Expo, Android, Tailwind setup, etc. | Mobile & Native | 5 |
| SK-035 | Swift Concurrency Pro | Swift async/await, actors, structured concurrency | Mobile & Native | 5 |
| SK-036 | Swift Testing Pro | Swift Testing framework (@Test, #expect) | Mobile & Native | 5 |
| SK-037 | SwiftUI Pro | SwiftUI patterns, @Observable, navigation | Mobile & Native | 5 |
| FS-025 | Flutter Expert | Flutter widgets, state, platform channels | Mobile & Native | 5 |
| FS-026 | React Native Expert | React Native architecture, native modules | Mobile & Native | 5 |
| SK-019 | Android Development | Android Kotlin, Jetpack Compose, Material 3 | Mobile & Native | 5 |
| CE-001 | SDD | Specification-Driven Development | Workflow & Meta | 6 |
| CE-002 | TDD | Test-Driven Development patterns | Workflow & Meta | 6 |
| CE-004 | Kaizen | Continuous improvement practices | Workflow & Meta | 6 |
| CE-006 | Git Workflows | Advanced git patterns and strategies | Workflow & Meta | 6 |
| CE-007 | DDD | Domain-Driven Design | Workflow & Meta | 6 |
| CE-008 | Documentation | Technical documentation patterns | Workflow & Meta | 6 |
| SK-012 | Skill Creator | Create new skills from capability gaps (superseded by SK-038 Self-Evolve) | Workflow & Meta | 6 |
| FS-034 | Code Quality Reviewer | Code quality review (superseded by CE-005 Code Review) | Workflow & Meta | 6 |
| SK-014 | Brand Guidelines | Brand consistency and guidelines management | Workflow & Meta | 6 |
| SK-015 | Discovery Doc | Technical discovery document creation | Workflow & Meta | 6 |
| SK-051 | Dev Cycle | Development lifecycle management | Workflow & Meta | 6 |
| CP-* | Compound Engineering Suite | Brainstorming, orchestrating-swarms, PR resolution, etc. | Workflow & Meta | 6 |
| FS-003 | JavaScript Expert | Modern JS patterns (superseded by SK-045 Advanced JS & TS) | Web (archived) | N/A |
| FS-065 | Motion iOS Config | Spring/haptic configs (superseded by SK-097 Native Motion Engine) | Mobile & Native | 5 |
| SK-010 | ClaudeKit Design | Claude brand assets — logos, CIP, banners, slides | Dormant — Document & Media | 7 |
| SK-032 | React Composition Patterns | Vercel React composition (covered by FS-021 React Expert) | Web (archived) | N/A |
| SK-033 | React Performance (Vercel) | Vercel perf patterns (covered by FS-021 React Expert) | Web (archived) | N/A |
| CE-003 | Reflexion | Auto-refinement triggers (inlined in CLAUDE.md) | Workflow & Meta | 6 |
| SK-038 | Self-Evolve | Autonomous skill/MCP creation (capability baked into workflow) | Workflow & Meta | 6 |
| SK-041 | Project Init | Auto-generate project CLAUDE.md (once-per-project use) | Workflow & Meta | 6 |
| SK-050 | Nano Banana (Gemini Image) | AI image gen/edit via Gemini (novelty) | Dormant — Document & Media | 7 |
| SK-072 | Write a PRD | PRD generation via interactive interviews | Workflow & Meta | 6 |
| SK-073 | PRD to Plan | PRD → phased implementation plans (overlaps Flow) | Workflow & Meta | 6 |
| SK-074 | Grill Me | Interview-driven plan/design validation | Workflow & Meta | 6 |
| SK-075 | Canvas Design | Visual art creation via design philosophy | Dormant — Document & Media | 7 |
| SK-006 | UX Design Intelligence | UX strategy, 67 design styles, 161 palettes, BM25 search | Web (archived) | N/A |
| SK-031 | Next.js Upgrade Guide | Upgrade Next.js to latest with migration steps | Web (archived) | N/A |
| SK-034 | Web Interface Guidelines | Web design compliance review | Web (archived) | N/A |
| SK-040 | Dream Consolidation | Multi-phase memory consolidation — orient, merge, prune | Workflow & Meta | 6 |
| SK-044 | GSAP Advanced (ScrollTrigger) | ScrollTrigger, Flip, Draggable, SplitText | Web (archived) | N/A |
| SK-077 | Subagent-Driven Dev | Task dispatch → fresh subagent → two-stage review | Workflow & Meta | 6 |
| SK-084 | CSS-First UI Engine | Container Queries, :has(), @layer, Popover+Anchor, OKLCH | Web (archived) | N/A |
| SK-085 | Streaming & Cache Architecture | Compositional cache, Activity, ViewTransition | Web (archived) | N/A |
| SK-087 | Modern Build Pipeline | Biome 2.0, Lightning CSS, Turbopack | Web (archived) | N/A |
| SK-088 | Tauri Desktop Engine | Rust-backed desktop apps, IPC, plugins, security | Desktop (archived) | N/A |
| SK-089 | Device Hardware Bridge | Unified camera, scanning, biometrics, sensors | Mobile & Native | 5 |
| SK-090 | Local-First Architecture | Offline-first CRDT sync — PowerSync, TinyBase | Mobile & Native | 5 |
| SK-091 | Edge Intelligence | On-device AI/ML: llama.rn, MediaPipe, sqlite-vec | Mobile & Native | 5 |
| SK-092 | Cross-Platform Monorepo | Turborepo for mobile + desktop + web code sharing | Mobile & Native | 5 |
| SK-093 | Anime.js v4 | Modular lightweight animation — stagger, text, scroll | Web (archived) | N/A |
| SK-094 | Barba.js Page Transitions | MPA smooth page transitions — GSAP orchestration | Web (archived) | N/A |
| SK-095 | Spline 3D Runtime | Design-to-web 3D — lazy loading, events, React/Next.js | Web (archived) | N/A |
| SK-096 | Cinematic Web Engine | Unified animation orchestration — SALA, Motion Tokens | Web (archived) | N/A |
| SK-097 | Native Motion Engine | Reanimated 4, Gesture Handler, Moti — 60fps native | Mobile & Native | 5 |
| SK-098 | Native Visual Canvas | Skia (GPU 2D), R3F native (3D), Rive, Lottie | Mobile & Native | 5 |
| SK-099 | Native Transition & Scroll | Shared elements, FlashList v2, Legend List | Mobile & Native | 5 |
| SK-100 | Native Sensory Design System | Motion tokens, haptic palette, sound design | Mobile & Native | 5 |
| FS-012 | SQL Expert | SQL optimization, strategic indexes | Data & ML | 4 |
| FS-028 | API Designer | REST, OpenAPI 3.1, RFC 7807 errors | Enterprise | 3 |
| FS-033 | Testing Strategist | Testing strategies and patterns | Enterprise | 3 |
| FS-035 | Debugging Expert | Systematic debugging | Enterprise | 3 |
| FS-052 | PostgreSQL Expert | PostgreSQL optimization and admin | Data & ML | 4 |
