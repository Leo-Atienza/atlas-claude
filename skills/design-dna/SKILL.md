...
license: MIT
metadata:
  vendored_from: "github:zanwei/design-dna@9d9d795"
---

> **In the ATLAS web-dev system:** the *extract-then-build* complement to impeccable (SK-102) and design-check (SK-127). Reach for it to measure a reference UI (image / screenshot / URL) into a portable 3-dimension profile — especially its `visual_effects` taxonomy (params / performance_tier / fallback). Feed the profile into impeccable's craft flow; impeccable's font procedure + bans still win. Vendored verbatim from zanwei/design-dna, MIT — see NOTICE.md.
