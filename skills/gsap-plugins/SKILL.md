...
license: MIT
metadata:
  vendored_from: "github:greensock/gsap-skills@aed9cfd"
---

> **In the ATLAS web-dev system:** the GSAP **plugin catalog** — complements SK-042 (GSAP Core: tweens/timelines/eases) and SK-044 (GSAP Advanced: ScrollTrigger + Flip/SplitText/Draggable). Reach here for ScrollSmoother, Observer, Inertia, MotionPath, DrawSVG/MorphSVG, ScrambleText, CustomEase/Wiggle/Bounce, Physics2D, GSDevTools, Pixi. Advanced ScrollTrigger techniques (batch/scrollerProxy/containerAnimation) + utils + performance are in `reference/`. Official GreenSock skill, MIT — see NOTICE.md. **All plugins are free now** (Webflow acquisition) — never add an `.npmrc` auth token or the private GreenSock registry.
