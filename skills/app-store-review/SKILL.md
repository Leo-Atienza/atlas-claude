...
metadata:
  author: safaiyeh
  version: "1.0.1"
  vendored_from: github:safaiyeh/app-store-review-skill@c2532321
---
...
> **In the ATLAS app-dev system:** this is the pre-submission compliance gate. Run it **before** `expo-deployment` / EAS Submit (or `fastlane release`) ships a build — Apple Review applies to Expo/EAS apps too. It pairs with `ship-verify` (SK-128, "is the artifact real?") and `tactile`/`mobile-app-design` (HIG *design* compliance, distinct from these *review* rules). Findings are advisory — report rejection risks with the rule number; don't auto-rewrite app logic.
