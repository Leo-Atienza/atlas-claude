...
> **In the ATLAS app-dev system:** this is the entry point for embedding RN/Expo into an **existing native iOS/Android app** (XCFramework/AAR packaging + host integration) — distinct from greenfield mobile work (`new-mobile-app`, `tactile`, `building-native-ui`). Covers both the bare-RN and Expo tracks. Native-integration scope, so it triggers on brownfield projects rather than the user's default Expo/EAS greenfield path.
