...
The modern solution uses the CSS Anchor Positioning API:
...
**Browser support**: Chrome 125+, Edge 125+. Not yet in Firefox or Safari — use a fallback.

### Portal Pattern (Frameworks)

Render dropdowns at the document root:
...
- **Arbitrary z-index values** like `z-index: 9999` — use a semantic z-index scale
- **Rendering dropdown inline** without an escape hatch from the parent's stacking context
...
Swipe-to-delete and similar gestures are invisible. Always provide a visible fallback. Hint at gestures with partial reveals or onboarding coach marks.
