---
name: gsap-advanced
description: "GSAP advanced — ScrollTrigger scroll-driven animation, plugins (Flip, Draggable, SplitText, DrawSVG, MorphSVG), and React/Vue/Svelte integration (useGSAP). Use for storytelling-scroll and marketing-page motion — see impeccable reference/motion-design.md § Scroll-driven motion for when scroll animation earns its place. For core tweens, timelines, and utilities use gsap (SK-042)."
user-invocable: true
---
...
> **2026 recommendation:** Prefer **Lenis** (SK-048, 3KB gzipped) over ScrollSmoother (~24KB) for smooth scrolling. Lenis uses native scroll, preserves `position: sticky`, and has better community support. Both integrate with GSAP's ticker — see **Orchestration: One Loop, One Owner (SALA)** below for the canonical pattern.
...
## Orchestration: One Loop, One Owner (SALA)

When ScrollTrigger shares a page with Lenis smooth scroll and/or R3F, run **exactly one rAF driver per page: GSAP's ticker**. Independent loops drift by a frame — scroll, tween, and 3D render each see different data, which reads as jitter.
...
// Lenis: disable its own rAF; GSAP's ticker drives it (ticker time is seconds)
const lenis = new Lenis({ autoRaf: false });
lenis.on("scroll", ScrollTrigger.update);
gsap.ticker.add((t) => lenis.raf(t * 1000));
gsap.ticker.lagSmoothing(0);

// R3F: <Canvas frameloop="never">, advanced from the same ticker
// const advance = useThree((s) => s.advance);
// useEffect(() => { const tick = () => advance(performance.now() / 1000);
//   gsap.ticker.add(tick); return () => gsap.ticker.remove(tick); }, [advance]);
```

This stack is **Lenis + ScrollTrigger** — not ScrollSmoother (see the ScrollSmoother note above). Full Lenis setup (ReactLenis provider, `scrollTo`, required CSS) → **lenis-smooth-scroll** (SK-048).

### Layer Ownership

Each element is animated by **exactly one** tool. Two drivers writing to one element = jank and property fights.

| Element | Owner |
|---|---|
| Simple reveals, hover states | CSS (transitions, scroll-driven `animation-timeline`) |
| Pinned / scrubbed / orchestrated sequences | GSAP ScrollTrigger |
| 3D meshes, cameras, shader uniforms | R3F — values may be tweened *by* ScrollTrigger, which is still one writer per property |

### Canvas Image-Sequence Scrub

Scroll-scrubbed frame sequence on a pinned `<canvas>` — preload via `createImageBitmap`, draw only when the frame index changes:

```javascript
const N = 120, frames = [];
const canvas = document.querySelector("#seq"), ctx2d = canvas.getContext("2d");
let last = -1, ready = false;
const url = (i) => `/frames/${String(i).padStart(4, "0")}.webp`;

const poster = new Image();                       // poster frame while loading
poster.src = url(0);
poster.onload = () => { if (!ready) ctx2d.drawImage(poster, 0, 0, canvas.width, canvas.height); };

ScrollTrigger.create({                            // lazy preload, one viewport early
  trigger: ".seq-section", start: "top bottom", once: true,
  onEnter: async () => {
    const bitmaps = await Promise.all(Array.from({ length: N }, (_, i) =>
      fetch(url(i)).then((r) => r.blob()).then((b) => createImageBitmap(b))));
    frames.push(...bitmaps);
    ready = true;
  }
});
...
ScrollTrigger.create({
  trigger: ".seq-section", start: "top top", end: "+=3000", pin: true, scrub: true,
  onUpdate: (self) => {
    if (!ready) return;
    const i = Math.min(N - 1, Math.floor(self.progress * N));
    if (i !== last) { ctx2d.drawImage(frames[i], 0, 0, canvas.width, canvas.height); last = i; }
  }
});
```

### Unified Reduced Motion

One `gsap.matchMedia()` block owns the decision — never per-tween checks. The reduce context registers **static finals**: content visible, no pins, last sequence frame drawn once, native scroll.

```javascript
const mm = gsap.matchMedia();

mm.add("(prefers-reduced-motion: no-preference)", () => {
  const sala = initSALA();          // the ticker wiring above, wrapped in a function
  initScrollAnimations();           // pins, scrubs, sequence scrub
  return () => sala.destroy();      // matchMedia cleanup if the preference flips
});

mm.add("(prefers-reduced-motion: reduce)", () => {
  gsap.set(".reveal", { autoAlpha: 1, clearProps: "transform" });  // content visible, no Lenis, no pins
  const final = new Image();                                       // sequence: last frame, drawn once
  final.src = url(N - 1);
  final.onload = () => ctx2d.drawImage(final, 0, 0, canvas.width, canvas.height);
});
```

For when scroll-driven motion earns its place at all, see **impeccable** `reference/scroll-storytelling.md`.

Orchestration patterns extracted from the archived **cinematic-web-engine** skill (SK-096, ATLAS-internal, archived 2026-04-27).
