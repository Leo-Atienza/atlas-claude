...
**Conditional Agents (run when PR matches criteria)**:
- **schema-drift-detector** - Detects unrelated schema changes by cross-referencing against included migrations. *Run if PR contains `db/migrate/*`, `db/schema.rb`, or equivalent migration files.*
- **data-migration-expert** - Validates ID mappings match production, checks rollback safety. *Run if PR modifies columns storing IDs, enums, mappings, or includes data backfill scripts.*
- **deployment-verification-agent** - Creates Go/No-Go deployment checklist with SQL verification queries. *Run if PR includes migrations + data transformations.*
- **agent-native-reviewer** - Verify new features are agent-accessible. *Run if PR adds user-facing features.*
- **code-simplicity-reviewer** - Check if code can be simplified. *Always run.*
...
- **If database migrations, schema files, or data backfills**: schema-drift-detector, data-migration-expert, deployment-verification-agent
- **If new user-facing features**: agent-native-reviewer
...
## Ultra-Thinking Deep Dive (for complex PRs)

For PRs touching >10 files or with high complexity, apply multi-perspective analysis:

### Stakeholder Perspectives
1. **Developer**: How easy is this to understand and modify? Are APIs intuitive?
2. **Operations**: How to deploy safely? What metrics/logs are available?
3. **End User**: Is the feature intuitive? Are error messages helpful?
4. **Security**: What's the attack surface? Compliance requirements?
...
- **Scale Testing**: 10x, 100x normal load behavior
