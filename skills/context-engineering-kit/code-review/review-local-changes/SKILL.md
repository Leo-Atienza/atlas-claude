...
Determine Applicable Reviews, then launch up to 8 parallel Sonnet agents to independently code review all local changes. The agents should do the following, then return a list of issues and the reason each issue was flagged (eg. CLAUDE.md or constitution.md adherence, bug, historical git context, etc.).
...
- **contracts-reviewer** - Analyze code contracts, including: type design and invariants (if new types added), API changes, data modeling, etc. *Run if types, API, or data modeling changed.*
- **test-coverage-reviewer** - Review test coverage quality and completeness. *Run if test files changed.*
- **schema-drift-detector** - Cross-reference schema changes against included migrations. *Run if PR contains database migrations or schema files.*
- **data-migration-expert** - Validate ID mappings, check rollback safety, verify data transformations. *Run if PR modifies columns storing IDs, enums, mappings, or includes data backfill scripts.*

**Always-on supplementary agents**:
- **agent-native-reviewer** - Verify new features are agent-accessible (if applicable)
- **code-simplicity-reviewer** - Check if code can be simplified
...
- **If database migrations, schema files, or data backfills**: schema-drift-detector, data-migration-expert
- **If new user-facing features added**: agent-native-reviewer

#### Launch Review Agents

**Parallel approach**:

- Launch all agents simultaneously (use worktrees for isolation if available)
...
## Ultra-Thinking Deep Dive (for complex changes)

For changes touching >10 files or with high complexity, apply ultra-thinking analysis:

### Stakeholder Perspective Analysis
Consider the change from multiple angles:
1. **Developer**: How easy is this to understand and modify? Are APIs intuitive? Is debugging straightforward?
2. **Operations**: How to deploy safely? What metrics/logs are available? Resource requirements?
3. **Security**: What's the attack surface? Compliance requirements? Data protection?

### Scenario Exploration
For complex changes, mentally walk through:
