...
| Path | Purpose |
|------|---------|
| `&lt;your-wiki-path&gt;\` | Vault root (bash: `<your-wiki-path>/`) |
...
| Update a page | `Edit` |
| List pages in a dir | `Bash`: `ls <your-wiki-path>/wiki/**/*.md` or `find <your-wiki-path>/wiki -name "*.md"` |
...
1. **List all pages** — `Bash`: `find <your-wiki-path>/wiki -name "*.md"`
