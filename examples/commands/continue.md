...
1. **Find the handoff file**:
   ...
   - Fallback: `~/.claude/handoffs/<cwd-slug>.md` — where `<cwd-slug>` is the current working directory with `/`, `\`, and `:` replaced by `_` (e.g. `~/.claude` → `<your-cwd-slug>`). This is the per-project handoff written by `session-stop.sh`; the old global `.last-session-handoff` was retired to avoid cross-project pollution.
