...
# Auto-detects the current OS user (portable across machines). Override with
# ATLAS_SCRUB_USERS="user1 user2" if several contributors' names leaked into
# tracked files.
_detected_user="$(whoami 2>/dev/null | sed 's#.*[\\/]##')"
LEAKED_USERNAMES="${ATLAS_SCRUB_USERS:-${_detected_user:-yourusername}}"
