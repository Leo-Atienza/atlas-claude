...
Read `~/.claude/topics/KNOWLEDGE-DIRECTORY.md` and scan existing G-ERR entries.
...
```bash
grep "^| G-ERR" ~/.claude/topics/KNOWLEDGE-DIRECTORY.md 2>/dev/null | tail -1
```
...
1. Append entry to `~/.claude/topics/KNOWLEDGE-PAGE-3-errors.md`:

```markdown
## G-ERR-{NNN}: {Name}
**Date**: {today's date} | **Tags**: #{tag1} #{tag2} #{tag3}

{Description of the mistake class — what went wrong, root cause, and the correct approach. Include code examples if helpful. Add prevention strategy (hook, rule, test, or workflow change).}

**Evidence**: {date}: {specific instance that triggered this entry}

---
```

2. Add row to KNOWLEDGE-DIRECTORY.md Errors table:
```
| G-ERR-{NNN} | {Name} | {tags} | {today's date} |
```
